...
public class MenuRepository : IMenuRepository
{
    ...
    {
        return await _context.Categories.Where(c => !c.Isdeleted).OrderBy(c => c.Id).Select(x => new CategoryViewModel
        {
            Id = x.Id,
            Name = x.Name,
    ...
    {
        List<ItemViewModel>? items = await _context.MenuItems
    ...
    public async Task<PagedResult<ItemViewModel>> GetItemsByCategoryAsync(int categoryId, int pageNumber, int pageSize, string searchTerm = "")
    {
        ...
        List<ItemViewModel>? items = await query
            .OrderBy(i => i.Id)
    ...
    {
        List<int>? assignedModifierGroups = _context.MappingMenuItemWithModifiers
        .Where(m => m.MenuItemId == id)
        .Select(m => m.ModifierGroupId)
        .ToList();

        List<ItemModifierGroupViewModel>? modifierGroups = _context.MappingMenuItemWithModifiers
                                                            .Where(m => m.MenuItemId == id)
        ...
        ItemViewModel? item = _context.MenuItems
            .Where(x => x.Id == id)
    ...
    public async Task<bool> SoftDeleteModifierGroupAsync(int id)
    {
        ...
        List<Modifiergroupmodifier>? modifierGroupModifier = await _context.Modifiergroupmodifiers.Where(mgm => mgm.Modifiergroupid == id).ToListAsync();
    ...
    {
        Modifiergroup? modifierGroup = _context.Modifiergroups
    ...
    public async Task<bool> AddModifierAsync(ModifierViewModel model)
    {
        ...
        {
            IEnumerable<Modifiergroupmodifier>? mappings = model.ModifierGroupIds.Select(groupId => new Modifiergroupmodifier
            {
    ...
    {
        ModifierViewModel? modifier = _context.Modifiers
            .Where(x => x.Id == id)
    ...
    public async Task<bool> SoftDeleteModifierAsync(int id)
    {
        ...
        List<Modifiergroupmodifier>? relatedModifierGroups = _context.Modifiergroupmodifiers.Where(mgm => mgm.Modifierid == id).ToList();
